...
SESSION_LABEL = "integration_events"
...
def validate_event_wav(
    ...
    *,
    ...
    assert path.exists(), f"missing event WAV: {path}"
    ...
    with wave.open(
        ...
        assert wav_file.getnchannels() == expected_channels

        assert wav_file.getsampwidth() == expected_sample_width
...
def validate_event_directory(
    ...
    *,
    ...
    event_directory_text = row["event_directory"]

    assert event_directory_text is not None

    event_directory = Path(event_directory_text)

    assert event_directory.exists()

    assert event_directory.is_dir()

    expected_frames = int(row["end_sample"]) - int(row["start_sample"])

    assert expected_frames > 0

    for node_id in NODE_IDS:
        validate_event_wav(
            event_directory / (f"node_{node_id}.wav"),
            expected_sample_rate=config.audio.sample_rate,
            expected_channels=config.audio.channels,
            expected_sample_width=config.audio.sample_width_bytes,
...
def validate_session_closed(
    ...
    *,
    ...
    assert stored_label == SESSION_LABEL
...
def validate_core_event(
    ...
    *,
    ...
    assert row["detector_event_id"] > 0

    assert row["start_sample"] >= 0

    assert row["end_sample"] > row["start_sample"]

    assert math.isfinite(row["peak_rms_dbfs"])
...
def validate_optional_pipeline_outputs(
    ...
    if rms_value is not None:
        assert math.isfinite(rms_value)
    ...
    if dominant_frequency is not None:
        assert math.isfinite(dominant_frequency)
    ...
    if classification_label is not None:
        ...
        assert len(classification_label) > 0

    if classification_confidence is not None:
        assert math.isfinite(classification_confidence)
...
async def run_event_integration(
    ...
    try:
        ...
        node_tasks = [
            asyncio.create_task(
                node.run(),
                name=(f"event-simulator-node-{node.node_id}"),
            )
            for node in nodes
        ]
        ...
        assert server.soundscape is not None

        assert server.soundscape.active_session_id == session_id
        ...
        event_row = await wait_for_persisted_event(
            server,
            timeout_s=EVENT_TIMEOUT_S,
        )
        ...
        soundscape_rows = server.events.database.get_soundscape_indices(
            session_id=session_id
        )

        assert soundscape_rows

        assert set(row["node_id"] for row in soundscape_rows) == set(NODE_IDS)

        assert all(
            row["end_sample"] - row["start_sample"] == config.audio.sample_rate
        ...
        while server.events.completed_events < 1:
            assert asyncio.get_running_loop().time() < deadline
            await asyncio.sleep(0.01)
        assert server.events.completed_events >= 1

        assert server.events.last_event_db_id is not None
        ...
        validate_optional_pipeline_outputs(event_row)
        ...
        assert server.soundscape.active_session_id is None
...
def test_three_node_event_detection_and_persistence(
    ...
    asyncio.run(run_event_integration(tmp_path))
...
if __name__ == "__main__":
    ...
    temporary_root = Path(tempfile.mkdtemp(prefix="wildlife-events-"))

    asyncio.run(run_event_integration(temporary_root))
