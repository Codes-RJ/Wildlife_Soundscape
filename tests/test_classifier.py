...
def make_features(
    *,
    ...
    return AcousticFeatures(
        ...
        mfcc_std=tuple(0.0 for _ in range(13)),
    )
...
def bird_features(
    *,
    ...
    return make_features(
        duration_s=5.7,
        dominant_frequency_hz=800.0,
        spectral_centroid_hz=8000.0,
        spectral_flatness=0.13,
        spectral_flux=0.037,
        zero_crossing_rate=0.037,
        spectral_bandwidth_hz=110.0,
        spectral_rolloff_hz=1730.0,
        snr_db=snr_db,
    )
...
def insect_features() -> AcousticFeatures:
    ...
    return make_features(
        duration_s=7.0,
        dominant_frequency_hz=12_200.0,
        spectral_centroid_hz=10_350.0,
        spectral_flatness=0.34,
        spectral_flux=0.0004,
        zero_crossing_rate=0.449,
        spectral_bandwidth_hz=2060.0,
        spectral_rolloff_hz=12_170.0,
        snr_db=15.0,
    )
...
def amphibian_features() -> AcousticFeatures:
    ...
    return make_features(
        duration_s=6.56,
        dominant_frequency_hz=3790.0,
        spectral_centroid_hz=1860.0,
        spectral_flatness=0.48,
        spectral_flux=0.0028,
        zero_crossing_rate=0.043,
        spectral_bandwidth_hz=8270.0,
        spectral_rolloff_hz=14_960.0,
        snr_db=15.0,
    )
...
def mammal_features() -> AcousticFeatures:
    ...
    return make_features(
        duration_s=0.138,
        dominant_frequency_hz=83.0,
        spectral_centroid_hz=103.0,
        spectral_flatness=0.638,
        spectral_flux=0.078,
        zero_crossing_rate=0.028,
        spectral_bandwidth_hz=1495.0,
        spectral_rolloff_hz=1192.0,
        snr_db=15.0,
    )
...
def noise_features() -> AcousticFeatures:
    ...
    return make_features(
        duration_s=3.23,
        dominant_frequency_hz=61.0,
        spectral_centroid_hz=9200.0,
        spectral_flatness=0.937,
        spectral_flux=0.0009,
        zero_crossing_rate=0.032,
        spectral_bandwidth_hz=8380.0,
        spectral_rolloff_hz=684.0,
        snr_db=15.0,
    )
...
def test_acoustic_class_values_are_stable() -> None:
    ...
    assert AcousticClass.BIRD.value == "bird"

    assert AcousticClass.INSECT.value == "insect"

    assert AcousticClass.AMPHIBIAN.value == "amphibian"

    assert AcousticClass.MAMMAL.value == "mammal"

    assert AcousticClass.NOISE.value == "noise"

    assert AcousticClass.UNKNOWN.value == "unknown"
...
def test_classification_result_accepts_no_secondary_candidate() -> None:
    ...
    result = ClassificationResult(
        ...
        reasons=("invalid feature vector",),
    )

    assert result.second_label is None
...
def test_classification_result_rejects_confidence_above_one() -> None:
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            margin=0.2,
            scores={AcousticClass.BIRD.value: 1.0},
            reasons=("test",),
        )
...
def test_classification_result_rejects_negative_confidence() -> None:
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            margin=0.1,
            scores={AcousticClass.BIRD.value: 0.5},
            reasons=("test",),
        )


def test_classification_result_requires_secondary_confidence_when_label_exists() -> (
    None
):
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            margin=0.2,
            scores={
                AcousticClass.BIRD.value: 0.8,
                AcousticClass.INSECT.value: 0.6,
            },
            reasons=("test",),
        )


def test_classification_result_requires_none_secondary_confidence_without_label() -> (
    None
):
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            margin=0.0,
            scores={AcousticClass.UNKNOWN.value: 1.0},
            reasons=("test",),
        )
...
def test_classification_result_rejects_same_primary_and_secondary_label() -> None:
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            second_confidence=0.6,
            margin=0.2,
            scores={AcousticClass.BIRD.value: 0.8},
            reasons=("test",),
        )
...
def test_classification_result_rejects_invalid_score_value() -> None:
    ...
    with pytest.raises(ValueError):
        ClassificationResult(
            ...
            second_confidence=0.4,
            margin=0.4,
            scores={AcousticClass.BIRD.value: 1.5},
            reasons=("test",),
        )
...
def test_classification_input_accepts_feature_only_input() -> None:
    ...
    assert classification_input.has_features

    assert not (classification_input.has_audio)

    assert classification_input.features is features
...
def test_classification_input_accepts_waveform_only_input() -> None:
    ...
    assert not (classification_input.has_features)

    assert classification_input.has_audio
...
def test_classifier_accepts_clear_synthetic_patterns(
    ...
    result = classifier.classify(features_factory())

    assert result.label == expected_label

    assert result.confidence >= classifier.MIN_ACCEPT_CONFIDENCE

    assert result.margin >= classifier.MIN_ACCEPT_MARGIN

    assert result.second_label is not None

    assert result.second_confidence is not None
...
def test_classifier_returns_complete_score_map() -> None:
    ...
    result = HeuristicClassifier().classify(bird_features())

    assert set(result.scores) == {
        acoustic_class.value for acoustic_class in AcousticClass
    }


def test_classifier_scores_are_bounded() -> None:

    result = HeuristicClassifier().classify(insect_features())
...
def test_accepted_result_score_matches_primary_label() -> None:
    ...
    assert result.confidence == pytest.approx(result.scores[result.label.value])
...
def test_accepted_result_second_confidence_matches_score_map() -> None:
    ...
    assert result.second_confidence == pytest.approx(
        result.scores[result.second_label.value]
    )
...
def test_invalid_zero_energy_features_return_unknown() -> None:
    ...
    assert result.label == AcousticClass.UNKNOWN
    ...
    assert result.scores[AcousticClass.UNKNOWN.value] == 1.0


def test_non_finite_feature_vector_returns_unknown() -> None:

    features = make_features(spectral_centroid_hz=float("nan"))
...
def test_extreme_low_snr_forces_unknown() -> None:
    ...
    features = bird_features(snr_db=-4.0)
    ...
    assert result.second_label == AcousticClass.NOISE

    assert result.second_confidence == pytest.approx(0.75)

    assert result.scores[AcousticClass.UNKNOWN.value] == 1.0

    assert result.scores[AcousticClass.NOISE.value] == pytest.approx(0.75)
...
def test_missing_snr_does_not_automatically_invalidate_classification() -> None:
    ...
    features = bird_features(snr_db=None)
...
def test_missing_snr_reduces_decision_confidence() -> None:
    ...
    known_snr = classifier.classify(bird_features(snr_db=15.0))

    missing_snr = classifier.classify(bird_features(snr_db=None))

    assert missing_snr.confidence < known_snr.confidence
...
def test_ambiguous_feature_vector_returns_unknown() -> None:
    ...
    assert result.second_label != AcousticClass.UNKNOWN
...
def test_flat_spectrum_adjustment_is_applied_before_final_ranking() -> None:
    ...
    assert result.label == AcousticClass.NOISE

    assert (
        result.scores[AcousticClass.NOISE.value]
        > result.scores[AcousticClass.INSECT.value]
    )

    assert result.confidence == pytest.approx(result.scores[AcousticClass.NOISE.value])
...
def test_accepted_classification_states_non_species_scope() -> None:
    ...
    assert any(
        "not species identification" in reason.lower() for reason in result.reasons
    )
...
def test_classifier_result_contains_current_classifier_identity() -> None:
    ...
    assert result.classifier_name == "heuristic_acoustic_classifier"

    assert result.classifier_version == "1.2"
...
def test_heuristic_backend_capabilities() -> None:
    ...
    assert backend.requires_features

    assert not (backend.requires_audio)

    assert backend.name == "heuristic_backend"
...
def test_heuristic_backend_rejects_missing_features() -> None:
    ...
    with pytest.raises(ValueError):
        backend.classify(classification_input)
...
def test_heuristic_backend_matches_direct_classifier() -> None:
    ...
    direct_classifier = HeuristicClassifier()

    backend = HeuristicClassifierBackend(direct_classifier)

    direct_result = direct_classifier.classify(features)
