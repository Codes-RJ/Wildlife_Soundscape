...
def test_header_and_crc_roundtrip() -> None:
    ...
    assert header.session_id == 20

    assert header.sample_index == 1024

    assert header.local_micros == 123

    assert header.i2s_error_count == 1
...
def test_packet_supports_protocol_integer_boundaries() -> None:
    ...
    raw = build_packet(
        ...
        local_micros=0xFFFFFFFF,
        i2s_error_count=0xFFFFFFFF,
        flags=0xFF,
        payload=payload,
    )
    ...
    assert header.node_id == 255

    assert header.sequence == 0xFFFFFFFF

    assert header.session_id == 0xFFFFFFFF

    assert header.sample_index == 0xFFFFFFFFFFFFFFFF

    assert header.local_micros == 0xFFFFFFFF

    assert header.i2s_error_count == 0xFFFFFFFF
...
def test_crc_mismatch_is_detected() -> None:
    ...
    corrupted_payload = bytearray(raw[HEADER_SIZE:])
...
def test_rejects_wrong_protocol_version() -> None:
    ...
    header_bytes[4] = (PROTOCOL_VERSION + 1) & 0xFF
...
def test_control_frame_roundtrip(
    ...
    packed = pack_control(frame)

    assert len(packed) == CONTROL_SIZE

    assert unpack_control(packed) == frame


def test_unpack_control_rejects_wrong_size() -> None:

    with pytest.raises(InvalidControlFrame):
        unpack_control(b"\x00" * (CONTROL_SIZE - 1))
...
def test_environment_roundtrip() -> None:
    ...
    environment = EnvironmentPayload(
        temperature_c=27.5,
        humidity_percent=58.25,
        pressure_hpa=1007.2,
    )

    parsed = parse_environment(pack_environment(environment))

    # Wire format uses float32, therefore approximate comparison is
    # appropriate after decoding.
    assert parsed.temperature_c == pytest.approx(
        environment.temperature_c,
        rel=1e-6,
    )

    assert parsed.humidity_percent == pytest.approx(
        environment.humidity_percent,
        rel=1e-6,
    )

    assert parsed.pressure_hpa == pytest.approx(
...
def test_sync_roundtrip() -> None:
    ...
    sync = SyncPayload(
        session_id=0x12345678,
        sync_id=7,
        sample_index=987654321,
        local_micros=123456,
    )

    assert parse_sync(pack_sync(sync)) == sync
...
def test_parse_sync_rejects_wrong_payload_size() -> None:
    ...
    with pytest.raises(ProtocolError):
        parse_sync(b"\x00" * (SYNC_STRUCT.size - 1))
...
def test_master_heartbeat_roundtrip() -> None:
    ...
    packed = pack_master_heartbeat(heartbeat)

    assert len(packed) == MASTER_HEARTBEAT_STRUCT.size
...
def test_slave_heartbeat_roundtrip() -> None:
    ...
    packed = pack_slave_heartbeat(heartbeat)

    assert len(packed) == SLAVE_HEARTBEAT_STRUCT.size
...
def test_heartbeat_role_mismatch_is_rejected() -> None:
    ...
    packed = pack_master_heartbeat(master_heartbeat)
