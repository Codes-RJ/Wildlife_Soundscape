...
def make_tone(
    ...
    *,
    ...
    time_s = sample_index / float(sample_rate)
...
def run_standard_filter(
    ...
    return bandpass_filter(
        ...
        low_hz=LOW_CUTOFF_HZ,
        high_hz=HIGH_CUTOFF_HZ,
        order=FILTER_ORDER,
    )
...
def test_bandpass_attenuates_out_of_band_tone() -> None:
    ...
    inside = make_tone(2000.0)

    outside = make_tone(50.0)

    y_inside = run_standard_filter(inside)

    y_outside = run_standard_filter(outside)

    assert calculate_rms(y_inside) > (5.0 * calculate_rms(y_outside))
...
def test_passband_tones_retain_significant_energy(
    ...
    signal = make_tone(frequency_hz)

    filtered = run_standard_filter(signal)

    input_rms = calculate_rms(signal)

    output_rms = calculate_rms(filtered)

    assert output_rms > (input_rms * 0.50)
...
def test_low_frequency_tones_are_attenuated(
    ...
    assert calculate_rms(filtered) < (calculate_rms(signal) * 0.25)
...
def test_high_frequency_tones_are_attenuated(
    ...
    assert calculate_rms(filtered) < (calculate_rms(signal) * 0.30)
...
def test_bandpass_retains_inband_component_from_mixed_signal() -> None:
    ...
    mixed = low + desired + high

    filtered = run_standard_filter(mixed)
    ...
    denominator = float(np.linalg.norm(filtered)) * float(np.linalg.norm(desired))
...
def test_filter_output_is_real_floating_point() -> None:
    ...
    assert not (np.iscomplexobj(filtered))


def test_filter_output_is_finite() -> None:

    signal = make_tone(3000.0)
...
def test_filter_does_not_modify_input_array() -> None:
    ...
    signal = make_tone(2500.0)

    original = signal.copy()

    run_standard_filter(signal)
...
def test_filter_is_deterministic() -> None:
    ...
    signal = make_tone(3500.0)

    first = run_standard_filter(signal)

    second = run_standard_filter(signal)
