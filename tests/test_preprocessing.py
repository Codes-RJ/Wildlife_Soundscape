...
def test_pcm16_conversion_is_contiguous() -> None:
    ...
    assert not (pcm.flags.c_contiguous)
...
def test_remove_dc_offset() -> None:
    ...
    signal = signal + np.float32(0.15)

    assert abs(float(np.mean(signal))) > 0.1
...
def test_remove_dc_does_not_destroy_ac_component() -> None:
    ...
    biased = original + np.float32(0.2)

    corrected = remove_dc_offset(biased)
...
def test_bandpass_rejects_low_frequency() -> None:
    ...
    attenuation_ratio = after / before
...
def test_peak_normalization_preserves_shape() -> None:
    ...
    signal = generate_sine(2000.0)

    normalized = peak_normalize(signal)
...
def test_peak_normalization_handles_near_silence() -> None:
    ...
    normalized = peak_normalize(near_silence)
...
def test_preprocessing_config_rejects_invalid_sample_rate() -> None:
    ...
    with pytest.raises(ValueError):
        PreprocessingConfig(sample_rate=0)


def test_preprocessing_config_rejects_zero_filter_order() -> None:

    with pytest.raises(ValueError):
        PreprocessingConfig(filter_order=0)
...
def test_preprocessing_config_rejects_invalid_model_target_peak(
    ...
    with pytest.raises(ValueError):
        PreprocessingConfig(model_target_peak=target_peak)
...
def test_preprocessing_config_accepts_expected_project_values() -> None:
    ...
    assert config.model_target_peak == 0.98
...
def test_complete_preprocessing_pipeline() -> None:
    ...
    assert result.raw_float.shape == pcm.shape

    assert result.amplitude_signal.shape == pcm.shape

    assert result.analysis_signal.shape == pcm.shape

    assert result.model_signal.shape == pcm.shape
    ...
    assert result.raw_float.flags.c_contiguous

    assert result.amplitude_signal.flags.c_contiguous

    assert result.analysis_signal.flags.c_contiguous

    assert result.model_signal.flags.c_contiguous
    ...
    model_peak = float(np.max(np.abs(result.model_signal)))
    ...
    expected_analysis_peak = float(np.max(np.abs(result.analysis_signal)))

    assert result.peak_before_normalization == pytest.approx(
    ...
    assert np.all(np.isfinite(result.raw_float))

    assert np.all(np.isfinite(result.amplitude_signal))

    assert np.all(np.isfinite(result.analysis_signal))

    assert np.all(np.isfinite(result.model_signal))
...
def test_preprocessing_raw_float_is_independent_of_input_pcm() -> None:
    ...
    saved_raw = result.raw_float.copy()
...
def test_preprocessing_keeps_amplitude_before_normalization() -> None:
    ...
    result = preprocess_event_audio(pcm)

    amplitude_peak = float(np.max(np.abs(result.amplitude_signal)))
...
def test_pipeline_handles_silence_without_nan() -> None:
    ...
    assert np.all(result.raw_float == 0.0)

    assert np.all(result.amplitude_signal == 0.0)

    assert np.all(result.model_signal == 0.0)

    assert result.peak_before_normalization == 0.0
