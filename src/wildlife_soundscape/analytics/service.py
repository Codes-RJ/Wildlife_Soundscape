...
def _normalize_spatial_bounds(
    ...
    if (
        ...
        raise TypeError(
            ("spatial_bounds must be ((x_min, y_min), (x_max, y_max)) or None.")
        )
...
def _activity_warnings(
    *,
    ...
    if total_events == 0:
        warnings.append(
            ("No acoustic events were available for temporal activity analysis.")
        )

    elif total_events < min_activity_events:
        warnings.append(
            (
...
def _spatial_warnings(
    ...
    *,
    ...
    if spatial.total_event_count > 0 and spatial.localized_event_count == 0:
        warnings.append(
            (
    ...
    if (
        spatial.localized_event_count < min_localized_events
        and spatial.localized_event_count > 0
    ):
        warnings.append(
            (
    ...
    if spatial.total_event_count > 0 and spatial.localization_coverage < 0.50:
        warnings.append(
            (
    ...
    transition_count = sum(
        transition.transition_count for transition in spatial.transitions
    )

    if spatial.localized_event_count >= 2 and transition_count < min_transitions:
        warnings.append(
            (
...
def _environmental_warnings(
    ...
    *,
    ...
    if not (environmental_rows_supplied):
        warnings.append(
            (
                "Environmental association analysis "
...
def _deduplicate_warnings(
    ...
    for warning in warnings:
        warning = _require_nonempty_string(
            warning,
            name="warning",
        )
...
def _resolve_report_window(
    ...
    if not isinstance(
        ...
        raise TypeError(("activity_window must be an AnalyticsTimeWindow."))
...
def build_research_analytics_report(
    ...
    *,
    environmental_rows: (Iterable[Any] | None) = None,
    # ------------------------------------------------------------------
    # TEMPORAL ANALYSIS
    # ------------------------------------------------------------------
    ...
    spatial_bounds = _normalize_spatial_bounds(spatial_bounds)

    if max_transition_gap_s is not None:
        max_transition_gap_s = _require_positive_finite(
            max_transition_gap_s,
            name="max_transition_gap_s",
        )
    ...
    if not isinstance(
        ...
        raise TypeError(("same_class_transitions_only must be bool."))

    alpha = _require_probability_open(
        alpha,
        name="alpha",
    )

    neutral_threshold = _require_probability_closed(
        neutral_threshold,
        name="neutral_threshold",
    )

    environmental_min_samples = _require_positive_int(
        environmental_min_samples,
        name="environmental_min_samples",
    )

    if environmental_min_samples < 3:
        raise ValueError(("environmental_min_samples must be at least 3."))
    ...
    environmental_fields = _normalize_optional_mapping(
        environmental_fields,
        default=DEFAULT_ENVIRONMENTAL_FIELDS,
        name="environmental_fields",
    )

    response_fields = _normalize_optional_mapping(
        response_fields,
        default=DEFAULT_RESPONSE_FIELDS,
    ...
    materialized_events = tuple(event_rows)
    ...
    spatial = build_spatial_summary(
        ...
        same_class_transitions_only=same_class_transitions_only,
    )
    ...
    environmental_rows_supplied = environmental_rows is not None
    ...
    if environmental_rows is None:
        environmental_associations = ()

    else:
        materialized_environmental_rows = tuple(environmental_rows)

        environmental_associations = build_environmental_associations(
            materialized_environmental_rows,
            environmental_fields=environmental_fields,
            response_fields=response_fields,
            alpha=alpha,
            min_samples=environmental_min_samples,
    ...
    behavior_indicators = build_behavior_indicators(
        activity=activity,
    ...
    warnings.extend(
        _environmental_warnings(
            environmental_associations,
            environmental_rows_supplied=environmental_rows_supplied,
        )
    )

    warnings.extend(_behavior_warnings(behavior_indicators))

    # ==================================================================
    # REPORT WINDOW
    # ==================================================================

    report_window = _resolve_report_window(activity.window)
