...
Examples of table sections may include:
...
Design principle
----------------
The exporter intentionally does not depend on exact field names inside
analytics result dataclasses.

Instead, it consumes the public `to_dict()` representation generated by
the analytics layer.

This keeps the exporter resilient if analytics models gain additional
research metrics later.


Database mode
-------------
The command-line interface can use the existing EventDatabase
read-side analytics methods:

    analytics_event_rows()
    analytics_environmental_bins()

The retrieved data is passed to:

    analytics.service.build_research_analytics_report()
...
Behaviour indicators are derived indicators rather than confirmed
...
def to_json_safe(
    ...
    Handles:
        ...
        datetime/date
        tuples/lists/sets
        numpy-style scalar values exposing item()
    """
...
def flatten_scalar_metrics(
    ...
    *,
    ...
    Lists are deliberately omitted from this scalar summary because they
    are exported separately as tabular CSV files.
    ...
    Output:

        {
            ...
            "activity.event_rate_per_hour": 4.3,
        }
    """
    ...
    if isinstance(
        ...
        for key, item in (
            ...
            # Lists are handled by table extraction.
...
def flatten_table_row(
    ...
    *,
    ...
    Nested lists remain JSON strings so no information is discarded.
    """
...
def discover_report_tables(
    ...
    *,
    ...
    A list whose members are dictionaries is exported as one CSV table.

    Lists of scalar values are also represented as a one-column table.
    """
    ...
    if isinstance(
        ...
        if all(
            ...
            # ----------------------------------------------------------
            # Also inspect nested structures inside each row.
            # ----------------------------------------------------------
            ...
            for index, item in enumerate(
                ...
                for nested_name, nested_rows in (
                    nested.items()
                ):
...
def build_research_report(
    ...
    *,
    ...
    Database access deliberately remains outside this function so this
    API can also be used with:
    ...
    return build_research_analytics_report(
        ...
        max_transition_gap_s=
            analytics.max_transition_gap_s,

        same_class_transitions_only=
            analytics.same_class_transitions_only,

        alpha=
            analytics.environmental_alpha,

        environmental_min_samples=
            analytics.environmental_min_samples,

        neutral_threshold=
            analytics.neutral_threshold,

        min_activity_events=
            analytics.min_activity_events,

        min_localized_events=
            analytics.min_localized_events,

        min_transitions=
            analytics.min_transitions,
    )
...
def build_and_export_research_metrics(
    ...
    *,
    ...
    """
    Convenience function for analytics + export.
    """
...
# ======================================================================
# SIGNATURE-AWARE METHOD INVOCATION
# ======================================================================


def call_supported_keywords(
    method,
    **candidate_arguments: Any,
):
    """
    Call one project method using only keyword arguments it supports.

    Why this helper exists
    ----------------------
    Database read APIs can evolve independently from this export tool.
    ...
    are supplied only when they are part of the current method
    signature.

    This avoids unnecessary coupling between exporter and database
    ...
    for name, value in (
        candidate_arguments.items()
    ):

        if (
            accepts_var_keywords
            or name
            in parameters
        ):
...
def create_event_database(
    ...
    """
    Construct EventDatabase while supporting common constructor styles.

    Current project code is expected to use a database-path constructor.
    """
    ...
    # ------------------------------------------------------------------
    # Positional fallback for the current compact database abstraction.
    # ------------------------------------------------------------------
...
def load_database_analytics_inputs(
    ...
    *,
    ...
    Environmental bins are deliberately used instead of only attaching
    weather to detected events.

    This preserves zero-event intervals and reduces event-conditioned
    sampling bias during environmental association analysis.
    """
    ...
    environmental_rows = (
        call_supported_keywords(
            ...
            bucket_seconds=
                config.analytics.bucket_seconds,

            node_id=
                config.analytics.environmental_node_id,
...
def export_database_session(
    ...
    *,
    ...
    One session is used intentionally.

    Combining separated acquisition sessions into one continuous
    temporal/spatial sequence could:

        introduce false downtime
        create false spatial transitions
        distort event-rate calculations
    """
    ...
    return build_and_export_research_metrics(
        event_rows,
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    source_group.add_argument(
        ...
        default=None,
        help=(
            "Build research analytics directly from one "
